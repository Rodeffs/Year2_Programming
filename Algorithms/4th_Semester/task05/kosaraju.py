...
Суть алгоритма в том, чтобы сначала пройти через поиск в глубину по
всем элемента графа, попутно запоминая порядок вхождения.
Т.е. вот к примеру граф:
A -> B -> C
По ходу алгоритма первый элемент здесь A, присвоим ему 1, далее
...
теперь у B будет значение 4, т.к. в него мы пошли после C. Аналогично, из
B в A, теперь у A значение 5.
Нужно теперь расположить элементы в порядке убывания их значений,
получаем массив меток. Далее разворачиваем граф (транспонируем его
матрицу) и просто делаем грубинний поиск по нему, начиная от первого
...
def depth(node, visited, graph, order):
    order += 1
